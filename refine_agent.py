...
class AnalystAgent:
    ...
    def answer_query(
        ...
        # Check for explicitly missing / out of scope years or metrics first (Negative testing)
        if any(yr in q_lower for yr in ["fy2025", "fy25", "fy2020", "fy20", "fy2021", "fy21", "r&d", "dividend"]):
            return {
        ...
        # 1. Margin Calculation (EBITDA Margin, Gross Margin, PAT Margin)
        ...
        # 6. Specific Entity / Structured Search (CIN, Auditors, Segments, Headcount, Score, Cell ref, etc.)
        ...
        # 7. Single Metric Search (Revenue, EBITDA, PAT, Debt, OCF, Assets, Net Worth, FCF, etc.)
            ...
            for m in ["Free Cash Flow", "Operating Cash Flow", "Cash and Cash Equivalents", "Cash & Cash Equivalents", "Gross Profit", "Total Liabilities", "Total Assets", "Total Debt", "Net Worth", "EBITDA", "PAT", "Revenue"]:
                if m.lower() in q_lower or (m == "Cash & Cash Equivalents" and "cash" in q_lower):
                    matched_metric = m
                    break
...
''')
print("AnalystAgent refined successfully.")
